"""Auto-generated client for target=kdcaCgmExpc
...
class GeneratedKdcacgmexpcClient(KoreanBaseClient):
    """Auto-generated client for target=kdcaCgmExpc.
...
# ── kdcaCgmExpc ──────────────────────────────────────
    def search_kdcaCgmExpcs(
    ...
    ) -> list[KdcacgmexpcList]:
        """[GENERATED] 질병관리청 법령해석 목록

        Args:
        search: 검색범위 (기본 : 1 법령해석명, 2: 본문검색)
        query: 검색범위에서 검색을 원하는 질의 (정확한 검색을 위한 문자열 검색 query='임상시험')
        ...
        Returns:
            List of KdcacgmexpcList instances.
            Response path: CgmExpc (item key not discovered)
        """
        params: dict = {"target": "kdcaCgmExpc", "type": "JSON"}
        ...
        data = self._parse_json_response(response, target="kdcaCgmExpc")
        ...
        if isinstance(items, dict):
            items = [items]
        return [KdcacgmexpcList.model_validate(item) for item in items]
    ...
    def get_kdcaCgmExpc_detail(
        ...
        return KdcacgmexpcDetail.model_validate(raw)
