"""Auto-generated client for target=mofCgmExpc
...
from lawpy.kr.generated._models_generated import MofcgmexpcDetail, MofcgmexpcList


class GeneratedMofcgmexpcClient(KoreanBaseClient):
    """Auto-generated client for target=mofCgmExpc.
...
# ── mofCgmExpc ──────────────────────────────────────
    def search_mofCgmExpcs(
        ...
        """[GENERATED] 해양수산부 법령해석 목록 조회

        Args:
        search: 검색범위 (기본 : 1 법령해석명, 2: 본문검색)
        query: 검색범위에서 검색을 원하는 질의 (정확한 검색을 위한 문자열 검색 query='폐기물')
        ...
        Returns:
            List of MofcgmexpcList instances.
            Response path: CgmExpc (item key not discovered)
        """
        params: dict = {"target": "mofCgmExpc", "type": "JSON"}
        ...
        data = self._parse_json_response(response, target="mofCgmExpc")
        ...
        if isinstance(items, dict):
            items = [items]
        return [MofcgmexpcList.model_validate(item) for item in items]
    ...
    def get_mofCgmExpc_detail(
        ...
        return MofcgmexpcDetail.model_validate(raw)
