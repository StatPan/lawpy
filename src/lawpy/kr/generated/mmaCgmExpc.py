"""Auto-generated client for target=mmaCgmExpc
...
from lawpy.kr.generated._models_generated import MmacgmexpcDetail, MmacgmexpcList


class GeneratedMmacgmexpcClient(KoreanBaseClient):
    """Auto-generated client for target=mmaCgmExpc.
...
# ── mmaCgmExpc ──────────────────────────────────────
    def search_mmaCgmExpcs(
        ...
        """[GENERATED] 병무청 법령해석 목록

        Args:
        search: 검색범위 (기본 : 1 법령해석명, 2: 본문검색)
        query: 검색범위에서 검색을 원하는 질의 (정확한 검색을 위한 문자열 검색 query='학력')
        ...
        Returns:
            List of MmacgmexpcList instances.
            Response path: CgmExpc (item key not discovered)
        """
        params: dict = {"target": "mmaCgmExpc", "type": "JSON"}
        ...
        data = self._parse_json_response(response, target="mmaCgmExpc")
        ...
        if isinstance(items, dict):
            items = [items]
        return [MmacgmexpcList.model_validate(item) for item in items]
    ...
    def get_mmaCgmExpc_detail(
        ...
        return MmacgmexpcDetail.model_validate(raw)
