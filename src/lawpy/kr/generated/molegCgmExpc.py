"""Auto-generated client for target=molegCgmExpc
...
class GeneratedMolegcgmexpcClient(KoreanBaseClient):
    """Auto-generated client for target=molegCgmExpc.
...
# ── molegCgmExpc ──────────────────────────────────────
    def search_molegCgmExpcs(
    ...
    ) -> list[MolegcgmexpcList]:
        """[GENERATED] 법제처 법령해석 목록

        Args:
        search: 검색범위 (기본 : 1 법령해석명, 2: 본문검색)
        query: 검색범위에서 검색을 원하는 질의 (정확한 검색을 위한 문자열 검색 query='법령')
        ...
        Returns:
            List of MolegcgmexpcList instances.
            Response path: CgmExpc (item key not discovered)
        """
        params: dict = {"target": "molegCgmExpc", "type": "JSON"}
        ...
        data = self._parse_json_response(response, target="molegCgmExpc")
        ...
        if isinstance(items, dict):
            items = [items]
        return [MolegcgmexpcList.model_validate(item) for item in items]
    ...
    def get_molegCgmExpc_detail(
        ...
        return MolegcgmexpcDetail.model_validate(raw)
