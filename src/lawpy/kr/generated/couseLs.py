...
class GeneratedCouselsClient(KoreanBaseClient):
    """Auto-generated client for target=couseLs.
...
# ── couseLs ──────────────────────────────────────
    def search_couseLss(
        ...
        data = self._parse_json_response(response, target="couseLs")
            ...
            for v in data.values():
                ...
                if isinstance(v, dict):
                    ...
                    if not raw:
                        raw = [v]
                    break
        return [CouselsList.model_validate(item) for item in raw]
