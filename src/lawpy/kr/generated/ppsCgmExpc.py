"""Auto-generated client for target=ppsCgmExpc
...
class GeneratedPpscgmexpcClient(KoreanBaseClient):
    """Auto-generated client for target=ppsCgmExpc.
...
# ── ppsCgmExpc ──────────────────────────────────────
    def search_ppsCgmExpcs(
        ...
        """[GENERATED] 조달청 법령해석 목록

        Args:
        search: 검색범위 (기본 : 1 법령해석명, 2: 본문검색)
        query: 검색범위에서 검색을 원하는 질의 (정확한 검색을 위한 문자열 검색 query='상금')
        ...
        Returns:
            List of PpscgmexpcList instances.
            Response path: CgmExpc (item key not discovered)
        """
        params: dict = {"target": "ppsCgmExpc", "type": "JSON"}
        ...
        data = self._parse_json_response(response, target="ppsCgmExpc")
        ...
        if isinstance(items, dict):
            items = [items]
        return [PpscgmexpcList.model_validate(item) for item in items]
    ...
    def get_ppsCgmExpc_detail(
        ...
        return PpscgmexpcDetail.model_validate(raw)
