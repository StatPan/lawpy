"""Auto-generated client for target=nfaCgmExpc
...
from lawpy.kr.generated._models_generated import NfacgmexpcDetail, NfacgmexpcList


class GeneratedNfacgmexpcClient(KoreanBaseClient):
    """Auto-generated client for target=nfaCgmExpc.
...
# ── nfaCgmExpc ──────────────────────────────────────
    def search_nfaCgmExpcs(
        ...
        """[GENERATED] 소방청 법령해석 목록

        Args:
        search: 검색범위 (기본 : 1 법령해석명, 2: 본문검색)
        query: 검색범위에서 검색을 원하는 질의 (정확한 검색을 위한 문자열 검색 query='공공기관')
        ...
        Returns:
            List of NfacgmexpcList instances.
            Response path: CgmExpc (item key not discovered)
        """
        params: dict = {"target": "nfaCgmExpc", "type": "JSON"}
        ...
        data = self._parse_json_response(response, target="nfaCgmExpc")
        ...
        if isinstance(items, dict):
            items = [items]
        return [NfacgmexpcList.model_validate(item) for item in items]
    ...
    def get_nfaCgmExpc_detail(
        ...
        return NfacgmexpcDetail.model_validate(raw)
