"""Auto-generated client for target=mafraCgmExpc
...
from lawpy.kr.generated._models_generated import MafracgmexpcDetail, MafracgmexpcList


class GeneratedMafracgmexpcClient(KoreanBaseClient):
    """Auto-generated client for target=mafraCgmExpc.
...
# ── mafraCgmExpc ──────────────────────────────────────
    def search_mafraCgmExpcs(
    ...
    ) -> list[MafracgmexpcList]:
        """[GENERATED] 농림축산식품부 법령해석 목록

        Args:
        search: 검색범위 (기본 : 1 법령해석명, 2: 본문검색)
        query: 검색범위에서 검색을 원하는 질의 (정확한 검색을 위한 문자열 검색 query='농지법')
        ...
        Returns:
            List of MafracgmexpcList instances.
            Response path: CgmExpc (item key not discovered)
        """
        params: dict = {"target": "mafraCgmExpc", "type": "JSON"}
        ...
        data = self._parse_json_response(response, target="mafraCgmExpc")
        ...
        if isinstance(items, dict):
            items = [items]
        return [MafracgmexpcList.model_validate(item) for item in items]
    ...
    def get_mafraCgmExpc_detail(
        ...
        return MafracgmexpcDetail.model_validate(raw)
