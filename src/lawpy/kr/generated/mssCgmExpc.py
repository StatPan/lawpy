"""Auto-generated client for target=mssCgmExpc
...
from lawpy.kr.generated._models_generated import MsscgmexpcDetail, MsscgmexpcList


class GeneratedMsscgmexpcClient(KoreanBaseClient):
    """Auto-generated client for target=mssCgmExpc.
...
# ── mssCgmExpc ──────────────────────────────────────
    def search_mssCgmExpcs(
        ...
        """[GENERATED] 중소벤처기업부 법령해석 목록

        Args:
        search: 검색범위 (기본 : 1 법령해석명, 2: 본문검색)
        query: 검색범위에서 검색을 원하는 질의 (정확한 검색을 위한 문자열 검색 query='학원')
        ...
        Returns:
            List of MsscgmexpcList instances.
            Response path: CgmExpc (item key not discovered)
        """
        params: dict = {"target": "mssCgmExpc", "type": "JSON"}
        ...
        data = self._parse_json_response(response, target="mssCgmExpc")
        ...
        if isinstance(items, dict):
            items = [items]
        return [MsscgmexpcList.model_validate(item) for item in items]
    ...
    def get_mssCgmExpc_detail(
        ...
        return MsscgmexpcDetail.model_validate(raw)
