"""Auto-generated client for target=mpmCgmExpc
...
class GeneratedMpmcgmexpcClient(KoreanBaseClient):
    """Auto-generated client for target=mpmCgmExpc.
...
# ── mpmCgmExpc ──────────────────────────────────────
    def search_mpmCgmExpcs(
        ...
        """[GENERATED] 인사혁신처 법령해석 목록

        Args:
        search: 검색범위 (기본 : 1 법령해석명, 2: 본문검색)
        query: 검색범위에서 검색을 원하는 질의 (정확한 검색을 위한 문자열 검색 query='징계위원회')
        ...
        Returns:
            List of MpmcgmexpcList instances.
            Response path: CgmExpc (item key not discovered)
        """
        params: dict = {"target": "mpmCgmExpc", "type": "JSON"}
        ...
        if isinstance(items, dict):
            items = [items]
        return [MpmcgmexpcList.model_validate(item) for item in items]
