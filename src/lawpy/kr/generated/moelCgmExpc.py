"""Auto-generated client for target=moelCgmExpc
...
from lawpy.kr.generated._models_generated import MoelcgmexpcDetail, MoelcgmexpcList


class GeneratedMoelcgmexpcClient(KoreanBaseClient):
    """Auto-generated client for target=moelCgmExpc.
...
# ── moelCgmExpc ──────────────────────────────────────
    def search_moelCgmExpcs(
    ...
    ) -> list[MoelcgmexpcList]:
        """[GENERATED] 고용노동부 법령해석 목록 조회

        Args:
        search: 검색범위 (기본 : 1 법령해석명, 2: 본문검색)
        query: 검색범위에서 검색을 원하는 질의 (정확한 검색을 위한 문자열 검색 query='퇴직')
        ...
        Returns:
            List of MoelcgmexpcList instances.
            Response path: CgmExpc.cgmExpc
        """
        params: dict = {"target": "moelCgmExpc", "type": "JSON"}
        ...
        data = self._parse_json_response(response, target="moelCgmExpc")
        ...
        if isinstance(items, dict):
            items = [items]
        return [MoelcgmexpcList.model_validate(item) for item in items]

    def get_moelCgmExpc_detail(
        self,
        id: int | None = None,
        lm: str | None = None,
        fields: str | None = None,
    ) -> MoelcgmexpcDetail:
        """[GENERATED] 고용노동부 법령해석 본문 조회
        ...
        return MoelcgmexpcDetail.model_validate(raw)
