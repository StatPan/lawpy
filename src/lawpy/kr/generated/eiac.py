"""Auto-generated client for target=eiac
...
class GeneratedEiacClient(KoreanBaseClient):
    """Auto-generated client for target=eiac.
...
# ── eiac ──────────────────────────────────────
    def search_eiacs(
        ...
        Returns:
            List of EiacList instances.
            Root key not discovered — using best-effort extraction
        """
        params: dict = {"target": "eiac", "type": "JSON"}
        ...
        data = self._parse_json_response(response, target="eiac")
            ...
            for v in data.values():
                ...
                if isinstance(v, dict):
                    ...
                    if not raw:
                        raw = [v]
                    break
        return [EiacList.model_validate(item) for item in raw]

    def get_eiac_detail(
        self,
        id: str | None = None,
    ) -> EiacDetail:
        """[GENERATED] 고용보험심사위원회 결정문 본문 조회
        ...
        return EiacDetail.model_validate(self._parse_json_response(response, target="eiac"))
