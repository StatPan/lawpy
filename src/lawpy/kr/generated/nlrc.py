...
class GeneratedNlrcClient(KoreanBaseClient):
    """Auto-generated client for target=nlrc.
...
# ── nlrc ──────────────────────────────────────
    def search_nlrcs(
        ...
        Returns:
            List of NlrcList instances.
            Root key not discovered — using best-effort extraction
        """
        params: dict = {"target": "nlrc", "type": "JSON"}
        ...
        data = self._parse_json_response(response, target="nlrc")
            ...
            for v in data.values():
                ...
                if isinstance(v, dict):
                    ...
                    if not raw:
                        raw = [v]
                    break
        return [NlrcList.model_validate(item) for item in raw]
    ...
    def get_nlrc_detail(
        ...
        return NlrcDetail.model_validate(self._parse_json_response(response, target="nlrc"))
