"""Auto-generated client for target=sfc
...
class GeneratedSfcClient(KoreanBaseClient):
    """Auto-generated client for target=sfc.
...
# ── sfc ──────────────────────────────────────
    def search_sfcs(
        ...
        sort: 정렬옵션 lasc : 안건명 오름차순 (default) ldes : 안건명 내림차순 nasc : 의결번호 오름차순 ndes : 의결번호 내림차순
        popyn: 상세화면 팝업창 여부(팝업창으로 띄우고 싶을 때만 'popYn=Y')

        Returns:
            List of SfcList instances.
            Root key not discovered — using best-effort extraction
        """
        params: dict = {"target": "sfc", "type": "JSON"}
        ...
        data = self._parse_json_response(response, target="sfc")
            ...
            for v in data.values():
                ...
                if isinstance(v, dict):
                    ...
                    if not raw:
                        raw = [v]
                    break
        return [SfcList.model_validate(item) for item in raw]

    def get_sfc_detail(
        self,
        id: str | None = None,
    ) -> SfcDetail:
        """[GENERATED] 증권선물위원회 결정문 본문 조회
        ...
        return SfcDetail.model_validate(self._parse_json_response(response, target="sfc"))
