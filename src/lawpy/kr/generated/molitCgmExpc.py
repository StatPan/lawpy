"""Auto-generated client for target=molitCgmExpc
...
class GeneratedMolitcgmexpcClient(KoreanBaseClient):
    """Auto-generated client for target=molitCgmExpc.
...
# ── molitCgmExpc ──────────────────────────────────────
    def search_molitCgmExpcs(
    ...
    ) -> list[MolitcgmexpcList]:
        """[GENERATED] 국토교통부 법령해석 목록 조회

        Args:
        search: 검색범위 (기본 : 1 법령해석명, 2: 본문검색)
        query: 검색범위에서 검색을 원하는 질의 (정확한 검색을 위한 문자열 검색 query='도로')
        ...
        Returns:
            List of MolitcgmexpcList instances.
            Response path: CgmExpc.cgmExpc
        """
        params: dict = {"target": "molitCgmExpc", "type": "JSON"}
        ...
        data = self._parse_json_response(response, target="molitCgmExpc")
        ...
        if isinstance(items, dict):
            items = [items]
        return [MolitcgmexpcList.model_validate(item) for item in items]

    def get_molitCgmExpc_detail(
        self,
        id: int | None = None,
        lm: str | None = None,
        fields: str | None = None,
    ) -> MolitcgmexpcDetail:
        """[GENERATED] 국토교통부 법령해석 본문 조회
        ...
        return MolitcgmexpcDetail.model_validate(raw)
