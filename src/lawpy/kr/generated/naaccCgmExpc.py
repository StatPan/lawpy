"""Auto-generated client for target=naaccCgmExpc
...
from lawpy.kr.generated._models_generated import NaacccgmexpcDetail, NaacccgmexpcList


class GeneratedNaacccgmexpcClient(KoreanBaseClient):
    """Auto-generated client for target=naaccCgmExpc.
...
# ── naaccCgmExpc ──────────────────────────────────────
    def search_naaccCgmExpcs(
    ...
    ) -> list[NaacccgmexpcList]:
        """[GENERATED] 행정중심복합도시건설청 법령해석 목록

        Args:
        search: 검색범위 (기본 : 1 법령해석명, 2: 본문검색)
        query: 검색범위에서 검색을 원하는 질의 (정확한 검색을 위한 문자열 검색 query='행복')
        ...
        Returns:
            List of NaacccgmexpcList instances.
            Response path: CgmExpc (item key not discovered)
        """
        params: dict = {"target": "naaccCgmExpc", "type": "JSON"}
        ...
        data = self._parse_json_response(response, target="naaccCgmExpc")
        ...
        if isinstance(items, dict):
            items = [items]
        return [NaacccgmexpcList.model_validate(item) for item in items]
    ...
    def get_naaccCgmExpc_detail(
        ...
        return NaacccgmexpcDetail.model_validate(raw)
