"""Auto-generated client for target=ppc
...
class GeneratedPpcClient(KoreanBaseClient):
    """Auto-generated client for target=ppc.
...
# ── ppc ──────────────────────────────────────
    def search_ppcs(
        ...
        sort: 정렬옵션 lasc : 안건명 오름차순 (default) ldes : 안건명 내림차순 dasc : 의결일자 오름차순 ddes : 의결일자 내림차순 nasc : 의안번호 오름차순 ndes : 의안번호 내림차순
        popyn: 상세화면 팝업창 여부(팝업창으로 띄우고 싶을 때만 'popYn=Y')

        Returns:
            List of PpcList instances.
            Root key not discovered — using best-effort extraction
        """
        params: dict = {"target": "ppc", "type": "JSON"}
        ...
        data = self._parse_json_response(response, target="ppc")
            ...
            for v in data.values():
                ...
                if isinstance(v, dict):
                    ...
                    if not raw:
                        raw = [v]
                    break
        return [PpcList.model_validate(item) for item in raw]

    def get_ppc_detail(
        self,
        id: str | None = None,
    ) -> PpcDetail:
        """[GENERATED] 개인정보보호위원회 결정문 본문 조회
        ...
        return PpcDetail.model_validate(self._parse_json_response(response, target="ppc"))
