"""Auto-generated client for target=mndCgmExpc
...
from lawpy.kr.generated._models_generated import MndcgmexpcDetail, MndcgmexpcList


class GeneratedMndcgmexpcClient(KoreanBaseClient):
    """Auto-generated client for target=mndCgmExpc.
...
# ── mndCgmExpc ──────────────────────────────────────
    def search_mndCgmExpcs(
        ...
        """[GENERATED] 국방부 법령해석 목록

        Args:
        search: 검색범위 (기본 : 1 법령해석명, 2: 본문검색)
        query: 검색범위에서 검색을 원하는 질의 (정확한 검색을 위한 문자열 검색 query='군종')
        ...
        Returns:
            List of MndcgmexpcList instances.
            Response path: CgmExpc (item key not discovered)
        """
        params: dict = {"target": "mndCgmExpc", "type": "JSON"}
        ...
        data = self._parse_json_response(response, target="mndCgmExpc")
        ...
        if isinstance(items, dict):
            items = [items]
        return [MndcgmexpcList.model_validate(item) for item in items]
    ...
    def get_mndCgmExpc_detail(
        ...
        return MndcgmexpcDetail.model_validate(raw)
