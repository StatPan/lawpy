"""Auto-generated client for target=oclt
...
from lawpy.kr.generated._models_generated import OcltDetail, OcltList


class GeneratedOcltClient(KoreanBaseClient):
    """Auto-generated client for target=oclt.
...
# ── oclt ──────────────────────────────────────
    def search_oclts(
        ...
        sort: 정렬옵션 lasc : 제목 오름차순 (default) ldes : 제목 내림차순
        popyn: 상세화면 팝업창 여부(팝업창으로 띄우고 싶을 때만 'popYn=Y')

        Returns:
            List of OcltList instances.
            Root key not discovered — using best-effort extraction
        """
        params: dict = {"target": "oclt", "type": "JSON"}
        ...
        data = self._parse_json_response(response, target="oclt")
            ...
            for v in data.values():
                ...
                if isinstance(v, dict):
                    ...
                    if not raw:
                        raw = [v]
                    break
        return [OcltList.model_validate(item) for item in raw]

    def get_oclt_detail(
        self,
        id: str | None = None,
    ) -> OcltDetail:
        """[GENERATED] 중앙토지수용위원회 결정문 본문 조회
        ...
        return OcltDetail.model_validate(self._parse_json_response(response, target="oclt"))
