"""Auto-generated client for target=kcgCgmExpc
...
from lawpy.kr.generated._models_generated import KcgcgmexpcDetail, KcgcgmexpcList


class GeneratedKcgcgmexpcClient(KoreanBaseClient):
    """Auto-generated client for target=kcgCgmExpc.
...
# ── kcgCgmExpc ──────────────────────────────────────
    def search_kcgCgmExpcs(
        ...
        """[GENERATED] 해양경찰청 법령해석 목록

        Args:
        search: 검색범위 (기본 : 1 법령해석명, 2: 본문검색)
        query: 검색범위에서 검색을 원하는 질의 (정확한 검색을 위한 문자열 검색 query='유조선')
        ...
        Returns:
            List of KcgcgmexpcList instances.
            Response path: CgmExpc (item key not discovered)
        """
        params: dict = {"target": "kcgCgmExpc", "type": "JSON"}
        ...
        data = self._parse_json_response(response, target="kcgCgmExpc")
        ...
        if isinstance(items, dict):
            items = [items]
        return [KcgcgmexpcList.model_validate(item) for item in items]
    ...
    def get_kcgCgmExpc_detail(
        ...
        return KcgcgmexpcDetail.model_validate(raw)
