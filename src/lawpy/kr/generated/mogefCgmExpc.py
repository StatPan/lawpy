"""Auto-generated client for target=mogefCgmExpc
...
class GeneratedMogefcgmexpcClient(KoreanBaseClient):
    """Auto-generated client for target=mogefCgmExpc.
...
# ── mogefCgmExpc ──────────────────────────────────────
    def search_mogefCgmExpcs(
    ...
    ) -> list[MogefcgmexpcList]:
        """[GENERATED] 성평등가족부 법령해석 목록

        Args:
        search: 검색범위 (기본 : 1 법령해석명, 2: 본문검색)
        query: 검색범위에서 검색을 원하는 질의 (정확한 검색을 위한 문자열 검색 query='청소년')
        ...
        Returns:
            List of MogefcgmexpcList instances.
            Response path: CgmExpc (item key not discovered)
        """
        params: dict = {"target": "mogefCgmExpc", "type": "JSON"}
        ...
        data = self._parse_json_response(response, target="mogefCgmExpc")
        ...
        if isinstance(items, dict):
            items = [items]
        return [MogefcgmexpcList.model_validate(item) for item in items]
    ...
    def get_mogefCgmExpc_detail(
        ...
        return MogefcgmexpcDetail.model_validate(raw)
