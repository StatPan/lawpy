"""Auto-generated client for target=motieCgmExpc
...
from lawpy.kr.generated._models_generated import MotiecgmexpcDetail, MotiecgmexpcList


class GeneratedMotiecgmexpcClient(KoreanBaseClient):
    """Auto-generated client for target=motieCgmExpc.
...
# ── motieCgmExpc ──────────────────────────────────────
    def search_motieCgmExpcs(
    ...
    ) -> list[MotiecgmexpcList]:
        """[GENERATED] 산업통상부 법령해석 목록

        Args:
        search: 검색범위 (기본 : 1 법령해석명, 2: 본문검색)
        query: 검색범위에서 검색을 원하는 질의 (정확한 검색을 위한 문자열 검색 query='공장')
        ...
        Returns:
            List of MotiecgmexpcList instances.
            Response path: CgmExpc (item key not discovered)
        """
        params: dict = {"target": "motieCgmExpc", "type": "JSON"}
        ...
        data = self._parse_json_response(response, target="motieCgmExpc")
        ...
        if isinstance(items, dict):
            items = [items]
        return [MotiecgmexpcList.model_validate(item) for item in items]
    ...
    def get_motieCgmExpc_detail(
        ...
        return MotiecgmexpcDetail.model_validate(raw)
