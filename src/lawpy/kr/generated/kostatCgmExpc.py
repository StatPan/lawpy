"""Auto-generated client for target=kostatCgmExpc
...
class GeneratedKostatcgmexpcClient(KoreanBaseClient):
    """Auto-generated client for target=kostatCgmExpc.
...
# ── kostatCgmExpc ──────────────────────────────────────
    def search_kostatCgmExpcs(
    ...
    ) -> list[KostatcgmexpcList]:
        """[GENERATED] 국가데이터처 법령해석 목록

        Args:
        search: 검색범위 (기본 : 1 법령해석명, 2: 본문검색)
        query: 검색범위에서 검색을 원하는 질의 (정확한 검색을 위한 문자열 검색 query='산업집적법')
        ...
        Returns:
            List of KostatcgmexpcList instances.
            Response path: CgmExpc (item key not discovered)
        """
        params: dict = {"target": "kostatCgmExpc", "type": "JSON"}
        ...
        data = self._parse_json_response(response, target="kostatCgmExpc")
        ...
        if isinstance(items, dict):
            items = [items]
        return [KostatcgmexpcList.model_validate(item) for item in items]
    ...
    def get_kostatCgmExpc_detail(
        ...
        return KostatcgmexpcDetail.model_validate(raw)
