"""Auto-generated client for target=dapaCgmExpc
...
from lawpy.kr.generated._models_generated import DapacgmexpcDetail, DapacgmexpcList


class GeneratedDapacgmexpcClient(KoreanBaseClient):
    """Auto-generated client for target=dapaCgmExpc.
...
# ── dapaCgmExpc ──────────────────────────────────────
    def search_dapaCgmExpcs(
    ...
    ) -> list[DapacgmexpcList]:
        """[GENERATED] 방위사업청 법령해석 목록

        Args:
        search: 검색범위 (기본 : 1 법령해석명, 2: 본문검색)
        query: 검색범위에서 검색을 원하는 질의 (정확한 검색을 위한 문자열 검색 query='제조')
        ...
        Returns:
            List of DapacgmexpcList instances.
            Response path: CgmExpc (item key not discovered)
        """
        params: dict = {"target": "dapaCgmExpc", "type": "JSON"}
        ...
        data = self._parse_json_response(response, target="dapaCgmExpc")
        ...
        if isinstance(items, dict):
            items = [items]
        return [DapacgmexpcList.model_validate(item) for item in items]
    ...
    def get_dapaCgmExpc_detail(
        ...
        return DapacgmexpcDetail.model_validate(raw)
