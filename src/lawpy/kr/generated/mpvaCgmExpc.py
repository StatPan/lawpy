"""Auto-generated client for target=mpvaCgmExpc
...
from lawpy.kr.generated._models_generated import MpvacgmexpcDetail, MpvacgmexpcList


class GeneratedMpvacgmexpcClient(KoreanBaseClient):
    """Auto-generated client for target=mpvaCgmExpc.
...
# ── mpvaCgmExpc ──────────────────────────────────────
    def search_mpvaCgmExpcs(
    ...
    ) -> list[MpvacgmexpcList]:
        """[GENERATED] 국가보훈부 법령해석 목록

        Args:
        search: 검색범위 (기본 : 1 법령해석명, 2: 본문검색)
        query: 검색범위에서 검색을 원하는 질의 (정확한 검색을 위한 문자열 검색 query='독립유공자')
        ...
        Returns:
            List of MpvacgmexpcList instances.
            Response path: CgmExpc (item key not discovered)
        """
        params: dict = {"target": "mpvaCgmExpc", "type": "JSON"}
        ...
        data = self._parse_json_response(response, target="mpvaCgmExpc")
        ...
        if isinstance(items, dict):
            items = [items]
        return [MpvacgmexpcList.model_validate(item) for item in items]
    ...
    def get_mpvaCgmExpc_detail(
        ...
        return MpvacgmexpcDetail.model_validate(raw)
