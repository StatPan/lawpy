"""Auto-generated client for target=moeCgmExpc
...
class GeneratedMoecgmexpcClient(KoreanBaseClient):
    """Auto-generated client for target=moeCgmExpc.
...
# ── moeCgmExpc ──────────────────────────────────────
    def search_moeCgmExpcs(
        ...
        """[GENERATED] 교육부 법령해석 목록

        Args:
        search: 검색범위 (기본 : 1 법령해석명, 2: 본문검색)
        query: 검색범위에서 검색을 원하는 질의 (정확한 검색을 위한 문자열 검색 query='수능')
        ...
        Returns:
            List of MoecgmexpcList instances.
            Response path: CgmExpc (item key not discovered)
        """
        params: dict = {"target": "moeCgmExpc", "type": "JSON"}
        ...
        data = self._parse_json_response(response, target="moeCgmExpc")
        ...
        if isinstance(items, dict):
            items = [items]
        return [MoecgmexpcList.model_validate(item) for item in items]
    ...
    def get_moeCgmExpc_detail(
        ...
        return MoecgmexpcDetail.model_validate(raw)
