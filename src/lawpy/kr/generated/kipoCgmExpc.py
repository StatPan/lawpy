"""Auto-generated client for target=kipoCgmExpc
...
from lawpy.kr.generated._models_generated import KipocgmexpcDetail, KipocgmexpcList


class GeneratedKipocgmexpcClient(KoreanBaseClient):
    """Auto-generated client for target=kipoCgmExpc.
...
# ── kipoCgmExpc ──────────────────────────────────────
    def search_kipoCgmExpcs(
    ...
    ) -> list[KipocgmexpcList]:
        """[GENERATED] 지식재산처 법령해석 목록

        Args:
        search: 검색범위 (기본 : 1 법령해석명, 2: 본문검색)
        query: 검색범위에서 검색을 원하는 질의 (정확한 검색을 위한 문자열 검색 query='상표')
        ...
        Returns:
            List of KipocgmexpcList instances.
            Response path: CgmExpc.cgmExpc
        """
        params: dict = {"target": "kipoCgmExpc", "type": "JSON"}
        ...
        data = self._parse_json_response(response, target="kipoCgmExpc")
        ...
        if isinstance(items, dict):
            items = [items]
        return [KipocgmexpcList.model_validate(item) for item in items]

    def get_kipoCgmExpc_detail(
        self,
        id: int | None = None,
        lm: str | None = None,
        fields: str | None = None,
    ) -> KipocgmexpcDetail:
        """[GENERATED] 지식재산처 법령해석 본문
        ...
        return KipocgmexpcDetail.model_validate(raw)
