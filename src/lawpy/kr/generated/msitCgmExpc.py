"""Auto-generated client for target=msitCgmExpc
...
from lawpy.kr.generated._models_generated import MsitcgmexpcDetail, MsitcgmexpcList


class GeneratedMsitcgmexpcClient(KoreanBaseClient):
    """Auto-generated client for target=msitCgmExpc.
...
# ── msitCgmExpc ──────────────────────────────────────
    def search_msitCgmExpcs(
    ...
    ) -> list[MsitcgmexpcList]:
        """[GENERATED] 과학기술정보통신부 법령해석 목록

        Args:
        search: 검색범위 (기본 : 1 법령해석명, 2: 본문검색)
        query: 검색범위에서 검색을 원하는 질의 (정확한 검색을 위한 문자열 검색 query='연구실')
        ...
        Returns:
            List of MsitcgmexpcList instances.
            Response path: CgmExpc (item key not discovered)
        """
        params: dict = {"target": "msitCgmExpc", "type": "JSON"}
        ...
        data = self._parse_json_response(response, target="msitCgmExpc")
        ...
        if isinstance(items, dict):
            items = [items]
        return [MsitcgmexpcList.model_validate(item) for item in items]
    ...
    def get_msitCgmExpc_detail(
        ...
        return MsitcgmexpcDetail.model_validate(raw)
