"""Auto-generated client for target=khsCgmExpc
...
class GeneratedKhscgmexpcClient(KoreanBaseClient):
    """Auto-generated client for target=khsCgmExpc.
...
# ── khsCgmExpc ──────────────────────────────────────
    def search_khsCgmExpcs(
        ...
        """[GENERATED] 국가유산청 법령해석 목록

        Args:
        search: 검색범위 (기본 : 1 법령해석명, 2: 본문검색)
        query: 검색범위에서 검색을 원하는 질의 (정확한 검색을 위한 문자열 검색 query='연말')
        ...
        Returns:
            List of KhscgmexpcList instances.
            Response path: CgmExpc.cgmExpc
        """
        params: dict = {"target": "khsCgmExpc", "type": "JSON"}
        ...
        if isinstance(items, dict):
            items = [items]
        return [KhscgmexpcList.model_validate(item) for item in items]
