...
class GeneratedFscClient(KoreanBaseClient):
    """Auto-generated client for target=fsc.
...
# ── fsc ──────────────────────────────────────
    def search_fscs(
        ...
        data = self._parse_json_response(response, target="fsc")
            ...
            for v in data.values():
                ...
                if isinstance(v, dict):
                    ...
                    if not raw:
                        raw = [v]
                    break
        return [FscList.model_validate(item) for item in raw]

    def get_fsc_detail(
        self,
        id: str | None = None,
    ) -> FscDetail:
        """[GENERATED] 금융위원회 결정문 본문 조회
        ...
        return FscDetail.model_validate(self._parse_json_response(response, target="fsc"))
