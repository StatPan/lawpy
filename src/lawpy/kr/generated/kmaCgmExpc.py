"""Auto-generated client for target=kmaCgmExpc
...
from lawpy.kr.generated._models_generated import KmacgmexpcDetail, KmacgmexpcList


class GeneratedKmacgmexpcClient(KoreanBaseClient):
    """Auto-generated client for target=kmaCgmExpc.
...
# ── kmaCgmExpc ──────────────────────────────────────
    def search_kmaCgmExpcs(
        ...
        """[GENERATED] 기상청 법령해석 목록

        Args:
        search: 검색범위 (기본 : 1 법령해석명, 2: 본문검색)
        query: 검색범위에서 검색을 원하는 질의 (정확한 검색을 위한 문자열 검색 query='태풍')
        ...
        Returns:
            List of KmacgmexpcList instances.
            Response path: CgmExpc (item key not discovered)
        """
        params: dict = {"target": "kmaCgmExpc", "type": "JSON"}
        ...
        data = self._parse_json_response(response, target="kmaCgmExpc")
        ...
        if isinstance(items, dict):
            items = [items]
        return [KmacgmexpcList.model_validate(item) for item in items]
    ...
    def get_kmaCgmExpc_detail(
        ...
        return KmacgmexpcDetail.model_validate(raw)
