"""Auto-generated tests for target=ttSpecialDecc."""

from unittest.mock import Mock

from lawpy.kr.generated._models_generated import TtspecialdeccDetail, TtspecialdeccList
from lawpy.kr.generated.ttSpecialDecc import GeneratedTtspecialdeccClient


def _make_client() -> GeneratedTtspecialdeccClient:
    return GeneratedTtspecialdeccClient(api_key="test_key")
...
class TestGeneratedTtspecialdeccClient:
    def test_search_returns_list_of_models(self):
        ...
        result = client.search_ttSpecialDeccs()
        assert isinstance(result, list)
        assert len(result) == 1
        assert isinstance(result[0], TtspecialdeccList)
    ...
    def test_search_passes_params(self):
        client = _make_client()
        client._make_request = Mock(return_value=_mock_response({"Decc": {"decc": [{"target": "val", "키워드": "val", "section": "val"}]}}))
        client.search_ttSpecialDeccs(search=1)
    ...
    def test_detail_returns_model(self):
        ...
        result = client.get_ttSpecialDecc_detail()
        assert isinstance(result, TtspecialdeccDetail)

    def test_detail_passes_params(self):
        client = _make_client()
        client._make_request = Mock(return_value=_mock_response({"Decc": {"특별행정심판재결례일련번호": "val", "사건명": "val", "사건번호": "val"}}))
        client.get_ttSpecialDecc_detail(id="1")
        call_params = client._make_request.call_args.kwargs.get("params", client._make_request.call_args[1].get("params", {}))
