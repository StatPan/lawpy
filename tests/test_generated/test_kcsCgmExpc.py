"""Auto-generated tests for target=kcsCgmExpc."""
...
def _make_client() -> GeneratedKcscgmexpcClient:
    return GeneratedKcscgmexpcClient(api_key="test_key")
...
class TestGeneratedKcscgmexpcClient:
    def test_search_returns_list_of_models(self):
        ...
        result = client.search_kcsCgmExpcs()
        assert isinstance(result, list)
        assert len(result) == 1
        assert isinstance(result[0], KcscgmexpcList)
    ...
    def test_search_passes_params(self):
        client = _make_client()
        client._make_request = Mock(return_value=_mock_response({"CgmExpc": {"cgmExpc": [{"target": "val", "키워드": "val", "section": "val"}]}}))
        client.search_kcsCgmExpcs(search=1)
    ...
    def test_detail_returns_model(self):
        client = _make_client()
        client._make_request = Mock(return_value=_mock_response({"CgmExpc": {"법령해석일련번호": "val", "업무분야": "val", "안건명": "val"}}))
        result = client.get_kcsCgmExpc_detail()
        assert isinstance(result, KcscgmexpcDetail)

    def test_detail_passes_params(self):
        client = _make_client()
        client._make_request = Mock(return_value=_mock_response({"CgmExpc": {"법령해석일련번호": "val", "업무분야": "val", "안건명": "val"}}))
        client.get_kcsCgmExpc_detail(id=1)
        call_params = client._make_request.call_args.kwargs.get("params", client._make_request.call_args[1].get("params", {}))
