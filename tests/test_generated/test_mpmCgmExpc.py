"""Auto-generated tests for target=mpmCgmExpc."""

from unittest.mock import Mock

from lawpy.kr.generated._models_generated import MpmcgmexpcDetail, MpmcgmexpcList
from lawpy.kr.generated.mpmCgmExpc import GeneratedMpmcgmexpcClient


def _make_client() -> GeneratedMpmcgmexpcClient:
    return GeneratedMpmcgmexpcClient(api_key="test_key")
...
class TestGeneratedMpmcgmexpcClient:
    def test_search_returns_list_of_models(self):
        ...
        result = client.search_mpmCgmExpcs()
        assert isinstance(result, list)
        if result:
            assert isinstance(result[0], MpmcgmexpcList)
    ...
    def test_search_passes_params(self):
        client = _make_client()
        client._make_request = Mock(return_value=_mock_response({"CgmExpc": [{"target": "val", "키워드": "val", "section": "val"}]}))
        client.search_mpmCgmExpcs(search=1)
    ...
    def test_detail_returns_model(self):
        ...
        result = client.get_mpmCgmExpc_detail()
        assert isinstance(result, MpmcgmexpcDetail)

    def test_detail_passes_params(self):
        client = _make_client()
        client._make_request = Mock(return_value=_mock_response({"CgmExpc": {"법령해석일련번호": "val", "안건명": "val", "안건번호": "val"}}))
        client.get_mpmCgmExpc_detail(id=1)
        call_params = client._make_request.call_args.kwargs.get("params", client._make_request.call_args[1].get("params", {}))
