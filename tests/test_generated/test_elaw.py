"""Auto-generated tests for target=elaw."""
...
def _make_client() -> GeneratedElawClient:
    return GeneratedElawClient(api_key="test_key")
...
class TestGeneratedElawClient:
    def test_search_returns_list_of_models(self):
        ...
        result = client.search_elaws()
        assert isinstance(result, list)
        if result:
            assert isinstance(result[0], ElawList)
    ...
    def test_detail_returns_model(self):
        ...
        result = client.get_elaw_detail()
        assert isinstance(result, ElawDetail)

    def test_detail_passes_params(self):
        client = _make_client()
        client._make_request = Mock(return_value=_mock_response({"LawSearch": {"OC": "val", "target": "val", "ID": "val"}}))
        client.get_elaw_detail(id="1")
        call_params = client._make_request.call_args.kwargs.get("params", client._make_request.call_args[1].get("params", {}))
