"""Auto-generated tests for target=ecc."""
...
def _make_client() -> GeneratedEccClient:
    return GeneratedEccClient(api_key="test_key")
...
class TestGeneratedEccClient:
    def test_search_returns_list_of_models(self):
        ...
        result = client.search_eccs()
        assert isinstance(result, list)
        assert len(result) == 1
        assert isinstance(result[0], EccList)
    ...
    def test_search_passes_params(self):
        client = _make_client()
        client._make_request = Mock(return_value=_mock_response({"result": [{"target": "val", "키워드": "val", "section": "val"}]}))
        client.search_eccs(search=1)
        call_params = client._make_request.call_args.kwargs.get("params", client._make_request.call_args[1].get("params", {}))
        assert "search" in call_params

    def test_detail_returns_model(self):
        client = _make_client()
        client._make_request = Mock(return_value=_mock_response({"결정문 일련번호": "val", "의결번호": "val", "사건명": "val"}))
        result = client.get_ecc_detail()
        assert isinstance(result, EccDetail)

    def test_detail_passes_params(self):
        client = _make_client()
        client._make_request = Mock(return_value=_mock_response({"결정문 일련번호": "val", "의결번호": "val", "사건명": "val"}))
        client.get_ecc_detail(id="1")
        call_params = client._make_request.call_args.kwargs.get("params", client._make_request.call_args[1].get("params", {}))
