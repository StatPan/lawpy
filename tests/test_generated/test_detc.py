...
class TestGeneratedDetcClient:
    ...
    def test_search_passes_mobileyn_param(self):
        client = _make_client()
        client._make_request = Mock(return_value=_mock_response({"DetcSearch": {"Detc": [{"target": "val", "키워드": "val", "section": "val"}]}}))
        client.search_detcs(mobileyn="Y")
        call_params = client._make_request.call_args.kwargs.get("params", client._make_request.call_args[1].get("params", {}))
        assert call_params["mobileYn"] == "Y"

    def test_search_accepts_root_list_fallback(self):
        client = _make_client()
        client._make_request = Mock(return_value=_mock_response({"DetcSearch": [{"target": "val", "키워드": "val", "section": "val"}]}))
        result = client.search_detcs()
        assert isinstance(result, list)
        assert len(result) == 1
        assert isinstance(result[0], DetcList)
    ...
    def test_detail_returns_model(self):
        client = _make_client()
        client._make_request = Mock(return_value=_mock_response({"DetcSearch": {"헌재결정례일련번호": "val", "종국일자": "val", "사건번호": "val"}}))
    ...
    def test_detail_passes_mobileyn_param(self):
        client = _make_client()
        client._make_request = Mock(return_value=_mock_response({"DetcSearch": {"헌재결정례일련번호": "val", "종국일자": "val", "사건번호": "val"}}))
        client.get_detc_detail(mobileyn="Y")
        call_params = client._make_request.call_args.kwargs.get("params", client._make_request.call_args[1].get("params", {}))
        assert call_params["mobileYn"] == "Y"
