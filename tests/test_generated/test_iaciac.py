"""Auto-generated tests for target=iaciac."""
...
def _make_client() -> GeneratedIaciacClient:
    return GeneratedIaciacClient(api_key="test_key")
...
class TestGeneratedIaciacClient:
    def test_search_returns_list_of_models(self):
        ...
        result = client.search_iaciacs()
        assert isinstance(result, list)
        if result:
            assert isinstance(result[0], IaciacList)
    ...
    def test_search_passes_params(self):
        client = _make_client()
        client._make_request = Mock(return_value=_mock_response({"result": [{"target": "val", "키워드": "val", "section": "val"}]}))
        client.search_iaciacs(search=1)
        call_params = client._make_request.call_args.kwargs.get("params", client._make_request.call_args[1].get("params", {}))
        assert "search" in call_params

    def test_detail_returns_model(self):
        client = _make_client()
        client._make_request = Mock(return_value=_mock_response({"결정문 일련번호": "val", "사건대분류": "val", "사건중분류": "val"}))
        result = client.get_iaciac_detail()
        assert isinstance(result, IaciacDetail)

    def test_detail_passes_params(self):
        client = _make_client()
        client._make_request = Mock(return_value=_mock_response({"결정문 일련번호": "val", "사건대분류": "val", "사건중분류": "val"}))
        client.get_iaciac_detail(id="1")
        call_params = client._make_request.call_args.kwargs.get("params", client._make_request.call_args[1].get("params", {}))
